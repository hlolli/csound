                            ...
                            void *name_arg, int32_t istring)
{
  ...
  if (istring) strncpy(fname, ((STRINGDAT*)name_arg)->data,MAXNAME-1) ;
    ...
    if (IsStringCode(*((cs_float*)name_arg)))
      strncpy(fname,csound->GetArgString(csound, *((cs_float*)name_arg)),MAXNAME-1);
...
static int32_t atsinfo_S(CSOUND *csound, ATSINFO *p)
{
  ...
  switch ((int32_t) CS_FLOAT2LRND(*p->ilocation)) {
  ...
  if (!swapped) {
    *p->ireturn = (cs_float) *ret_data;
    return OK;
  }
  /* otherwise do byteswapping */
  *p->ireturn = (cs_float) bswap(ret_data);
  return OK;
}
...
static void FetchPartial(ATSREAD *p, cs_float *buf, cs_float position)
{
  ...
  if (frame == p->maxFr) {
    if (p->swapped == 1) {
      buf[0] = (cs_float) bswap(frm_1);       /* calc amplitude */
      buf[1] = (cs_float) bswap(frm_1 + 1);   /* calc freq */
    }
    else {
      buf[0] = (cs_float) *frm_1;             /* calc amplitude */
      buf[1] = (cs_float) *(frm_1 + 1);       /* calc freq */
    }
    return;
  }
  ...
  buf[0] = (cs_float) (frm1amp + frac * (frm2amp - frm1amp));    /* calc amp. */
  buf[1] = (cs_float) (frm1freq + frac * (frm2freq - frm1freq)); /* calc freq */
}
 ...
 * ATSREADNOISE
 */
static cs_float FetchNzBand(ATSREADNZ *p, cs_float position)
{
  cs_float   frac;               /* the distance in time we are between frames */
  ...
  if (UNLIKELY(frame == p->maxFr))
    return (cs_float) frm1val;

  frm_2 = frm_1 + p->frmInc;
  frac = position - frame;
  frm2val = (p->swapped == 1) ? bswap(frm_2) : *frm_2;

  return (cs_float) (frm1val + frac * (frm2val - frm1val));  /* calc energy */
}
 ...
 * ATSADD
 */
static  void    FetchADDPartials(ATSADD *, ATS_DATA_LOC *, cs_float);
static  void    AtsAmpGate(ATS_DATA_LOC *, int32_t, FUNC *, cs_double);
...
static int32_t atsaddset(CSOUND *csound, ATSADD *p)
{
  ...
  memsize =   (int32_t) (*p->iptls) * sizeof(ATS_DATA_LOC)
    + (int32_t) (*p->iptls) * sizeof(cs_double)
    + (int32_t) (*p->iptls) * sizeof(cs_float);
  ...
  p->oscphase = (cs_double *) (p->buf + (int32_t) (*p->iptls));
  p->oldamps = (cs_float *) (p->oscphase + (int32_t) (*p->iptls));
  /* byte swap if necessary */
  if (p->swapped == 1) {
    p->maxFr = (int32_t) bswap(&atsh->nfrms) - 1;
    p->timefrmInc = bswap(&atsh->nfrms) / bswap(&atsh->dur);
...
{
  cs_float   frIndx;
  cs_float   *ar, amp, fract, v1, *ftab,a,inca, *oldamps = p->oldamps, incrf;
  FUNC    *ftp;
  cs_double  phasef;
  ...
  for (i = 0; i < numpartials; i++) {
    lobits = ftp->lobits;
    amp = csound->Get0dBFS(csound) * (cs_float) p->buf[i].amp;
    phase = CS_FLOAT2LONG(*oscphase);
    phasef = *oscphase;
    ...
    if(floatph)
      ...
      inc = CS_FLOAT2LONG(p->buf[i].freq * CS_SICVT * *p->kfmod);
     
    for (n=offset; n<nsmps; n++) {
      if(!floatph) {
        ftab = ftp->ftable + (phase >> lobits);
        v1 = *ftab++;
        fract = (cs_float) PFRAC(phase);
...
static void AtsAmpGate(            /* adaption of PvAmpGate by Richard Karpen */
                       ...
                       FUNC *ampfunc, cs_double MaxAmpInData)
{
  int32_t  j;
  ...
  for (j = 0; j < npartials; ++j) {
    ...
    buf[j].amp *= (cs_double) *(ampfunc->ftable + mapPoint);
  }
}
...
static void randiats_setup(CSOUND *csound, cs_float freq, RANDIATS *radat, cs_float sr)
{
  radat->size = (int32_t) CS_FLOAT2LRND(sr / freq);
  radat->cnt = 0;
...
static cs_float randiats(CSOUND *csound, RANDIATS *radat)
{
  cs_float   output;
  ...
  output = (((cs_float) (radat->a2 - radat->a1) / (cs_float) radat->size)
            * (cs_float) radat->cnt) + (cs_float) radat->a1;
  radat->cnt++;
  return (FL(1.0) - ((cs_float) output * (FL(2.0) / (cs_float) 0x7FFFFFFF)));
}
...
static const cs_double freqs[25]= {
...
static int32_t atsaddnzset(CSOUND *csound, ATSADDNZ *p)
{
  ...
  if (p->swapped == 1) {
    ...
    p->timefrmInc = bswap(&atsh->nfrms) / bswap(&atsh->dur);
    n_partials = (int32_t) bswap(&atsh->npartials);
    p->winsize = (cs_float) bswap(&atsh->winsz);
  }
  else {
    p->maxFr = (int32_t) atsh->nfrms - 1;
    p->timefrmInc = atsh->nfrms / atsh->dur;
    n_partials = (int32_t) atsh->npartials;
    p->winsize = (cs_float) atsh->winsz;
  }
  ...
  memcpy(p->nfreq, freqs, 25*sizeof(cs_double));
  /* p->nfreq[0] = 100.0; */
  /* p->nfreq[1] = 100.0; */
  /* p->nfreq[2] = 100.0; */
  /* p->nfreq[3] = 100.0; */
  ...
  /* initialise phase */
  memset(p->oscphase, '\0', 25*sizeof(cs_double));
  /* p->oscphase[0] = 0.0; */
  /* p->oscphase[1] = 0.0; */
  /* p->oscphase[2] = 0.0; */
  /* p->oscphase[3] = 0.0; */
  ...
  for (i = 0; i < 25; i++) {
    ...
    if (i == synthme && nsynthed < p->bands) { /* synthesize cosine */
      amp = csound->Get0dBFS(csound)*
        SQRT((p->buf[i] / (p->winsize*(cs_float)ATSA_NOISE_VARIANCE)));
      for (n=offset; n<nsmps; n++) {
...
static void fetchSINNOIpartials(ATSSINNOI *, cs_float);

static int32_t atssinnoiset_common(CSOUND *csound, ATSSINNOI *p,
                                    int32_t istring)
{
  char atsfilname[MAXNAME];
  ATSSTRUCT *atsh;
  int32_t i, type, npartials, count, first, step, width;
  size_t slots, memsize;

  p->swapped = load_atsfile(csound, p, &p->atsmemfile, atsfilname,
                            p->ifileno, istring);
  if (UNLIKELY(p->swapped < 0))
    return NOTOK;
  ...
  atsh = (ATSSTRUCT *) p->atsmemfile->beginp;
  if (p->swapped) {
    p->maxFr = bswap(&atsh->nfrms) - 1;
    p->timefrmInc = bswap(&atsh->nfrms) / bswap(&atsh->dur);
    npartials = (int32_t) bswap(&atsh->npartials);
    p->winsize = bswap(&atsh->winsz);
    type = (int32_t) bswap(&atsh->type);
  }
  else {
    p->maxFr = atsh->nfrms - 1;
    p->timefrmInc = atsh->nfrms / atsh->dur;
    npartials = (int32_t) atsh->npartials;
    p->winsize = atsh->winsz;
    type = (int32_t) atsh->type;
  }

  /* Check before converting or allocating. The last selected partial is
     first + (count - 1) * step, not first + count * step. */
  if (UNLIKELY(!(*p->iptls >= 0 && (cs_double) *p->iptls <= npartials) ||
               !(*p->iptloffset >= 0 &&
                 (cs_double) *p->iptloffset <= npartials) ||
               !(*p->iptlincr >= 1 &&
                 (cs_double) *p->iptlincr <= (INT32_MAX / 3 + 0.0))))
    goto range_error;
  ...
  if (UNLIKELY(count > 0 &&
               (first >= npartials ||
                count - 1 > (npartials - 1 - first) / step)))
    goto range_error;

  slots = count > 0 ? (size_t) count : 1;
  if (UNLIKELY(slots > SIZE_MAX / (sizeof(ATS_DATA_LOC) +
                                  sizeof(RANDIATS) + 2 * sizeof(cs_double))))
    goto range_error;
  memsize = slots * (sizeof(ATS_DATA_LOC) + sizeof(RANDIATS) +
                     2 * sizeof(cs_double));
  if (p->auxch.auxp == NULL || memsize > p->auxch.size)
    csound->AuxAlloc(csound, memsize, &p->auxch);
  p->oscbuf = (ATS_DATA_LOC *) p->auxch.auxp;
  p->randinoise = (RANDIATS *) (p->oscbuf + slots);
  p->oscphase = (cs_double *) (p->randinoise + slots);
  p->nzbuf = p->oscphase + slots;
  ...
  p->partialinc = width * step;
  p->firstband = type >= 3 ? 1 + width * npartials : -1;
  p->frmInc = 1 + width * npartials + (type >= 3 ? 25 : 0);
  p->prFlg = 1;
  ...
  memset(p->noiphase, 0, sizeof(p->noiphase));
...
{
  return atssinnoiset_common(csound, p, 0);
}

static int32_t atssinnoiset_S(CSOUND *csound, ATSSINNOI *p)
{
  return atssinnoiset_common(csound, p, 1);
}
...
static int32_t atssinnoi(CSOUND *csound, ATSSINNOI *p)
{
  ...
  /* Preserve the existing fixed-band noise synthesis. The old per-partial
     energy conversion was unused: this fetch overwrote its result. */
  if (p->firstband != -1)
    FetchADDNZbands(p->partials < 25 ? p->partials : 25, p->firstband,
                    p->datastart, p->frmInc, p->maxFr,
  ...
  if (p->firstband != -1) {
    ...
    for (i = 0; i < p->partials; i++) {
      ...
      ar = p->aoutput;
      amp = oscbuf[i].amp;
      freq = (cs_float) oscbuf[i].freq * *p->kfreq;
      inc = TWOPI * freq * CS_ONEDSR;
      nzamp = i < 25 ?
        sqrt(p->nzbuf[i] / (p->winsize * ATSA_NOISE_VARIANCE)) : 0.0;
      for (n=offset; n<nsmps;n++) {
        /* calc sine wave */
        sinewave = cos(phase);
        phase += inc;
        ...
        ar[n] += csound->Get0dBFS(csound) *
          (cs_float)(amp * sinewave * *p->ksinamp + noise **p->knzamp);
      }
      p->oscphase[i] = phase;
    }
    ...
    for (i = 0; i < p->partials; i++) {
      ...
      for (n=offset; n<nsmps;n++) {
        ...
        phase += inc;
        /* calc output */
        ar[n] += csound->Get0dBFS(csound) * (cs_float)sinewave * *p->ksinamp;
      }
      p->oscphase[i] = phase;
    }
...
static void fetchSINNOIpartials(ATSSINNOI *p, cs_float position)
{
  ...
  for (i = 0; i < p->partials; i++) {
    size_t index = p->firstpartial + (size_t) i * p->partialinc;
    cs_double amp0, amp1, freq0, freq1;
    if (p->swapped) {
...
static int mycomp(const void *p1, const void *p2)
{
  ...
  cs_double a1f = a1->freq;
  cs_double a2f = a2->freq;
  if (a1f < a2f)
    return -1;
  else if (a1f == a2f)
    return 0;
...
static int32_t atsinterpread(CSOUND *csound, ATSINTERPREAD *p)
{
  ...
  for (i = 0; i < (int32_t) *(atsbufreadaddr->iptls); i++) {
    ...
    if ((cs_float) ((atsbufreadaddr->table[i + 1]).freq) > *p->kfreq)
      break;
  }
  if (i == 0) {
    *p->kamp = FL(0.0);
  ...
  *p->kamp =
    (cs_float) ((atsbufreadaddr->table[i]).amp +
             frac * ((atsbufreadaddr->table[i + 1]).amp -
                     (atsbufreadaddr->table[i]).amp));
  /* *p->kamp = (cs_float) (atsbufreadaddr->table[i]).amp; */
  return OK;
 err1:
  return csound->PerfError(csound, &(p->h),
                           "%s", Str("ATSINTERPREAD: you must have an "
...
static int32_t atscrossset(CSOUND *csound, ATSCROSS *p)
{
  ...
  memsize =   (int32_t)(*p->iptls) *
    (sizeof(ATS_DATA_LOC) + sizeof(cs_double) + sizeof(cs_float)) ;
  /* allocate space if we need it */
  /* need room for a buffer and an array of oscillator phase increments */
  if (p->auxch.auxp == NULL || p->auxch.size >= (uint32_t)memsize)
    csound->AuxAlloc(csound, (size_t) memsize, &p->auxch);
...
static void ScalePartials(
                          ...
                          cs_float cbufamp,      /* the amplitude for the current buffer */
                          ATS_DATA_LOC *tbuf, /* the table buffer */
                          int32_t tbufnp,     /* the table buffer's n partials */
                          cs_float tbufamp,      /* the amp of the table buffer */
                          cs_float kthresh )
{
  IGN(csound);
  cs_float   tempamp;            /* hold the cbufn amp for a bit */
  cs_float   frac;               /* for interpilation */
...
{
  ATSBUFREAD  *atsbufreadaddr;
  cs_float   frIndx, *oldamps = p->oldamps, a, inca;
  cs_float   *ar, amp, fract, v1, *ftab, incrf;
  cs_double  phasef;
  FUNC    *ftp;
  int32    lobits, phase, inc;
  cs_double  *oscphase;
  ...
  for (i = 0; i < numpartials; i++) {
    lobits = ftp->lobits;
    amp = csound->Get0dBFS(csound) * (cs_float) p->buf[i].amp;
    phase = CS_FLOAT2LONG (oscphase[i]);
    phasef = *oscphase;
    ar = p->aoutput;         /* ar is a pointer to the audio output */
    inca = (amp-oldamps[i])/nsmps;
    /* put in * kfmod */

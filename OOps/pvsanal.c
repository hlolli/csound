...
int32_t pvssanalset(CSOUND *csound, PVSANAL *p)
{
  ...
  if (p->fsig->frame.auxp==NULL ||
      (size_t)CS_KSMPS*(N+2)*sizeof(cs_float) > p->fsig->frame.size)
    csound->AuxAlloc(csound, (size_t)CS_KSMPS*(N+2)*sizeof(cs_float),
                    &p->fsig->frame);
  else memset(p->fsig->frame.auxp, 0,
              (size_t)CS_KSMPS*(N+2)*sizeof(cs_float));
  /* Space for remembering samples */
  if (p->input.auxp==NULL ||
      (size_t)N*sizeof(cs_float) > p->input.size)
  ...
  if (p->trig.auxp==NULL ||
      (size_t)(2*NB)*sizeof(cs_double) > p->trig.size)
    csound->AuxAlloc(csound, (size_t)(2*NB)*sizeof(cs_double), &p->trig);
  {
...
#define SDFT_THREE_TERM_WINDOW(a0, a1, a2) do {                         \
  const cs_float w0 = FL(a0), w1 = FL(a1)*FL(0.5), w2 = FL(a2)*FL(0.5);   \
...
int32_t pvssynth(CSOUND *csound, PVSYNTH *p)
{
  ...
  const cs_double nyquist_sign = ((N/2) & 1) ? -1.0 : 1.0;
